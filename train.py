...
import onnxruntime as ort
from utils import DirectDataset, Classify_Task, Classifier, NewPad, RandomRotate
# # SimpleClassifier
...
img_transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor(), NewPad(new_w=256, new_h=256),
...
direct_dataset = DirectDataset(root_path, img_transform, joint_transform=RandomRotate)  # 问题，如何将区分trainval呢？
...
scheduler = optim.lr_scheduler.MultiStepLR(optimizer, milestones=[300, 800], gamma=0.1)  # optimizer其实就是solver, solvers/optimizers
...
# # export
# ## save pth
torch.save(classifier.state_dict(), "save_20250514.pth")

classifier_loaded = Classifier(num_classes=4)
classifier_loaded.load_state_dict(torch.load("save_20250514.pth", weights_only=True))  #  map_location=torch.device("cuda")，并非是指model在cuda上的意思
...
# export pth
classifier_task.export("save_20250514.onnx")

ort_sess = ort.InferenceSession('save_20250514.onnx')
for i, (image, label) in enumerate(iter(val_dataloader)):
